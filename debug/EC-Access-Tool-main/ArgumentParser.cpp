...
ArgumentParser::ArgumentParser(int argc, char** argv) : mDriverType(Driver::DriverType::eUnInit), mParsedCommands(), mDump(false), mFilter(false) {
    if (argc < 2) {
...
BOOL ArgumentParser::dump() const {
    return mDump;
}

BOOL ArgumentParser::filter() const {
    return mFilter;
}
